...
class ModelSkeleton:
  ...
  def __init__(self, mc):
    ...
    self.cls_labels = tf.placeholder(
        tf.float32, [mc.BATCH_SIZE, mc.ANCHORS, mc.CLASSES], name='cls_labels')
    self.cls_label_mask = tf.placeholder(
        tf.float32, [mc.BATCH_SIZE, mc.ANCHORS, mc.CLASSES], name='cls_label_mask')
    
    # Tensor used to represent bounding box deltas.
    self.bbox_delta = tf.placeholder(
        tf.float32, [mc.BATCH_SIZE, mc.ANCHORS, 4], name='bbox_delta')
    self.bbox_delta_mask = tf.placeholder(
        tf.float32, [mc.BATCH_SIZE, mc.ANCHORS, 4], name='bbox_delta_mask')
  ...
  def _add_loss_graph(self):
    ...
    with tf.variable_scope('class_regression') as scope:
      ...
      self.class_loss = tf.reduce_sum(
              (self.cls_labels*(-tf.log(self.pred_class_probs+mc.EPSILON))
               + (1-self.cls_labels)*(-tf.log(1-self.pred_class_probs+mc.EPSILON)))
              * self.cls_label_mask * mc.LOSS_COEF_CLASS,name='class_loss')
      
      tf.add_to_collection('losses', self.class_loss)

    with tf.variable_scope('bounding_box_regression') as scope:
      self.bbox_loss = tf.reduce_sum(
              mc.LOSS_COEF_BBOX * tf.square(
                  self.bbox_delta_mask*(self.pred_box_delta-self.bbox_delta)),name='bbox_loss')
